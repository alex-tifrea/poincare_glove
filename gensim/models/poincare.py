...
# Modified by Bonaventure Dossou: <femipancrace.dossou@gmail.com>

# ============================================ Function modified ===================================================== #
# added encoding = "utf-8" for csv reader for the Link Prediction and Reconstruction file reading                      #
# modified find_matching_terms() to bring best possibilities in the model's vocabulary closed to the input word        #
# instead of considering only words starting by the input words, leading to "KeyError" error                           #
# ============================================= End Modifications ==================================================== #
...
class LinkPredictionEvaluation(object):
    ...
    def __init__(self, train_path, test_path, embedding):
        ...
        for relation_type, data_file in data_files.items():
            with utils.open(data_file, 'r', encoding="utf-8") as f:
                reader = csv.reader(f, delimiter='\t')
                for row in reader:
                    assert len(row) == 2, 'Hypernym pair has more than two items'
                    item_1_index = embedding_vocab[row[0]].index
...
class LexicalEntailmentEvaluation(object):
    ...
    def __init__(self, filepath):
        ...
        """
        expected_scores = {}
        with utils.open(filepath, 'r', encoding="utf-8") as f:
    ...
    def find_matching_terms(trie, word):
        """ # Old -- Find terms in the `trie` beginning with the `word` --- #.
            # New -- Find terms in the model's vocabulary matching the best `word` -- #
        Parameters
        ----------
        trie : :class:`pygtrie.Trie`
        ...
        def intersection(word1_list, word2_list):  # takes the intersections of two lists of words
            words_list = [value for value in word1_list if value in word2_list]
            return words_list

        # matches = trie.items('%s' % word) # old version

        matches_ = [(''.join(key), len(intersection(list(set(key)), list(set(word))))) for key, value in trie.items() if
                    len(intersection(list(set(key)), list(set(word)))) != 0]
        max_length = max([_[1] for _ in matches_])
        sorted_matches = sorted(matches_, key=lambda x: x[1], reverse=True)
        best_matches = [match[0] for match in matches_ if match[1] == max_length]
        # matching_terms = [''.join(key_chars) for key_chars, value in matches] # old version
        return best_matches
